-module(graphviz).
-export([digraph/1, graph/2, delete/0, add_node/1, add_edge/2, graph_server/2, to_dot/1, to_file/2]).
...
graph(Id,PC) ->
   register(graph_server, spawn(?MODULE, graph_server, [{Id, {graph, "--"}, [] ,[], []},PC])).
...
graph_server(Graph,PC) ->
   receive
      {add_node, Id} -> 
         graph_server(add_node(Graph, Id),PC);

      {add_edge, NodeOne, NodeTwo} -> 
         graph_server(add_edge(Graph, NodeOne, NodeTwo, PC),PC);
